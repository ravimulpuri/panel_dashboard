...
PROJECT=panel_dashboard
CONTAINER_NAME="panel_dashboard_bash_${USER}"  ## Ensure this is the same name as in docker-compose.yml file
...
ci-black: ## Test lint compliance using black. Config in pyproject.toml file
	docker exec $(CONTAINER_NAME) black --check /mnt/panel_dashboard

ci-flake8: ## Test lint compliance using flake8. Config in tox.ini file
	docker exec $(CONTAINER_NAME) flake8 /mnt/panel_dashboard

ci-test:  ## Runs unit tests using pytest
	docker exec $(CONTAINER_NAME) pytest /mnt/panel_dashboard

ci-test-interactive:  ## Runs unit tests using pytest, and gives you an interactive IPDB session at the first failure
	docker exec -it $(CONTAINER_NAME) pytest /mnt/panel_dashboard  -x --pdb --pdbcls=IPython.terminal.debugger:Pdb

ci: ci-black ci-flake8 ci-test ## Check black, flake8, and run unit tests
	@echo "CI sucessful"

isort: ## Runs isort to sorts imports
	docker exec $(CONTAINER_NAME) isort -rc /mnt/panel_dashboard

black: ## Runs black auto-linter
	docker exec $(CONTAINER_NAME) black /mnt/panel_dashboard
...
dev-stop: ## Spin down active containers
	docker-compose -f docker/docker-compose.yml --project-name panel_dashboard down
